...
(defmethod move ((ball ball))
  (let ((radius (radius ball)))
    (let ((x (~> ball coordinates gui-box:x))
          ...
          (wr (- (~> *pong-game* game-area bottom-right gui-box:x) radius))
          (wl (+ (~> *pong-game* game-area top-left gui-box:x) radius))
          (wt (+ (~> *pong-game* game-area top-left gui-box:y) radius))
          (wb (- (~> *pong-game* game-area bottom-right gui-box:y) radius)))
      (cond ((>= x wr)
